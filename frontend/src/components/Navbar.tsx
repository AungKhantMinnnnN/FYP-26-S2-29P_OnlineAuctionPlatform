    ...
    <nav className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/85">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 gap-3">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            {role === 'admin' && (
              <Link to="/admin-dashboard" className="rounded-full px-3 py-2 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-100 hover:text-accent-700 dark:text-slate-300 dark:hover:bg-slate-900 dark:hover:text-accent-300">Admin</Link>
            )}
            {user ? (
              <button onClick={logout} className="ml-2 rounded-full bg-slate-900 px-4 py-2 text-sm font-semibold text-white shadow-soft transition-all hover:-translate-y-0.5 dark:bg-slate-100 dark:text-slate-950">Logout</button>
            ) : (
              <>
      ...
      {open && (
        <div className="md:hidden border-t border-slate-200/80 bg-white/95 px-4 py-4 space-y-2 shadow-xl shadow-slate-900/5 backdrop-blur dark:border-slate-800 dark:bg-slate-950/95">
          ...
          {user ? (
            <>
              <Link to={role === 'admin' ? '/admin-dashboard' : '/browse'} className="block rounded-xl px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-900" onClick={() => setOpen(false)}>{role === 'admin' ? 'Admin Dashboard' : 'Products'}</Link>
              <button className="block w-full rounded-xl px-3 py-2 text-left text-sm font-semibold text-red-600 hover:bg-red-50" onClick={() => { logout(); setOpen(false); }}>Logout</button>
            </>
          ) : (
            <>
