-- Syncs a few tables to match the backend models.

BEGIN;

-- notifications: add title, drop legacy columns
ALTER TABLE notifications ADD COLUMN IF NOT EXISTS title VARCHAR(255);

-- Guarded: 'type' may already be gone on a re-run
DO $$
BEGIN
...
-- admin_logs: add details, drop legacy columns
ALTER TABLE admin_logs ADD COLUMN IF NOT EXISTS details TEXT;

DO $$
BEGIN
    -- Copy metadata JSON as text into details before dropping the column
    IF EXISTS (SELECT 1 FROM information_schema.columns
