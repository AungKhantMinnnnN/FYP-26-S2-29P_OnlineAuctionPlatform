...
export default function WatchlistPage() {
  ...
  const initialIds = useMemo(() => WatchList.filter((item) => item.UserID === userId).map((item) => item.ListingID), [userId])
  const [watchIds, setWatchIds] = useState(initialIds)
  const watchedAuctions = auctions.filter((auction) => watchIds.includes(auction.id))
          ...
          <div className="rounded-2xl bg-slate-50 p-5 dark:bg-slate-950/60">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-3xl font-bold text-slate-950 dark:text-slate-50">{watchedAuctions.length}</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
            {watchedAuctions.map((auction) => (
              <div key={auction.id} className="space-y-3">
                ...
                <button onClick={() => setWatchIds((ids) => ids.filter((id) => id !== auction.id))} className="inline-flex w-full items-center justify-center gap-2 rounded-full border border-red-200 bg-red-50 px-4 py-2 text-sm font-semibold text-red-700 transition-all hover:-translate-y-0.5 hover:bg-red-100 dark:border-red-900/60 dark:bg-red-950/30 dark:text-red-300 dark:hover:bg-red-950/50">
