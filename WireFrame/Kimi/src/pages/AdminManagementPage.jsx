...
export default function AdminManagementPage() {
  ...
  const configs = {
    users: { headers: ['User ID', 'Name', 'Email', 'Role', 'Status'], rows: users.map(u => [u.user_id, u.full_name, u.email, u.role, <StatusBadge status={u.status} />]) },
    ...
    categories: { headers: ['Category ID', 'Name', 'Active Listings'], rows: categories.map((c, i) => [i + 1, c, auctions.filter(a => a.category === c).length]) },
    ...
    bids: { headers: ['Bid ID', 'Listing', 'Bidder', 'Amount', 'Status'], rows: bids.map(b => [b.bid_id, b.listing_title, b.bidder_id, `$${b.bid_amount.toFixed(2)}`, <StatusBadge status={b.status} />]) },
  }
